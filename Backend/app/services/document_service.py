...
async def upload_document(
    ...
    # Runs AFTER this request returns a response — upload feels instant to the user,
    # while chunking/embedding happens in the background.
    background_tasks.add_task(process_document_task, document.id)

    return DocumentResponse.model_validate(document)


async def list_documents(session: AsyncSession, only_public: bool) -> List[DocumentResponse]:
    documents = await document_repo.get_all(session, only_public=only_public)
...
async def delete_document(session: AsyncSession, document_id: int) -> dict:
    document = await document_repo.get_by_id(session, document_id)
    if not document:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")
